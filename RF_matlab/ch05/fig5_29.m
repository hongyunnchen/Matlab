%
%   Computation of attenuation profile for the bandstop filter obtained
...
w_bandpass=w_c./(w_U-w_L).*(w_real/w_c-w_c./w_real);
w=1./w_bandpass;
IL=10*log10(1+(1-8*w.^2+8*w.^4).^2);

plot(w_real/(2*pi)/1e9,IL);
axis([0 2 0 30]);
title('Attenuation of 3dB Chebyshev bandstop filter');
xlabel('Frequency {\itf}, GHz');
ylabel('Attenuation, dB');
%print -deps 'fig5_29.eps'
